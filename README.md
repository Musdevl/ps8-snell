...
| `backup` | mongo-tools / cron | — | Nightly AES-256 encrypted MongoDB backups (`snell-backup`) |
...
BACKUP_ENCRYPTION_KEY="..."      # generated by ./backup.sh --genkey
```
...
| `BACKUP_ENCRYPTION_KEY` | AES-256 key for the nightly database backups — generate it with `./backup.sh --genkey`. See [Backups](#backups). |
...
./restore-backup.sh      # restore the database (asks for confirmation)
```

The key lives in `services/.env` as `BACKUP_ENCRYPTION_KEY`.

> ⚠️ `services/.env` is **not** versioned. Keep a copy of that key somewhere safe:
> without it, no backup can ever be read again.

Full documentation: [`services/backup/README.md`](./services/backup/README.md)
