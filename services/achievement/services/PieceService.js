...
export function convertIntGridToBin(grid) {
...
export function findProtectors(grid, color) {
    const aliveShields = [];

    for (let i = 0; i < grid[0].length; i++) {
        for (let j = 0; j < grid[0].length; j++) {
            if (getColor(grid[i][j]) === color && getPiece(grid[i][j]) === PIECE.PROTECTOR) {
                aliveShields.push(grid[i][j]);
            }
        }
    }
